...
<ul {role} aria-live={ariaLive} aria-relevant="additions">
    ...
    <li class={ notification.type } style={`--duration: ${ notification.duration || 4000 }ms`} use:notificationTimer={notification}
        in:fly="{{ y: 5, duration: 200 }}" out:fade="{{ duration: 100 }}">
