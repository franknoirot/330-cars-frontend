...
const emailSnippets = {
    ...
    receiptSection: (trip: TripEmailConfig) => `<h2>Full receipt</h2>
        <table>
            <tbody>
    ...
    carDriverSection: (trip: TripEmailConfig) => `<h2>Car & driver</h2>
        <table>
            <tbody>
                <tr>
                    <th>Car</th>
...
export function tripConfirmationEmail(trip: TripEmailConfig) : EmailConfig {
    return {
        ...
        ${ emailSnippets.carDriverSection }
        ${ emailSnippets.receiptSection }
        </body>
        </html>
        `
    }
...
/**
 * Builds an API-friendly config for the trip resend email sent when using the email form
 * on the Reservation page.
 * @param {TripEmailConfig} trip 
 * @returns {EmailConfig}
 */
 export function tripResendEmail(trip: TripEmailConfig) : EmailConfig {
    return {
        toEmail: trip.driver.email,
        subject: 'Your requested reservation reprint with 330 Cars',
        text: `Your trip ID is ${ trip.id }, and is scheduled for ${ trip.pickup }.`,
        html: `<html>
        ${ emailSnippets.head }
        <body>
        <h1>Your 330 Cars reservation has been reprinted</h1>
        <p>Your reservation ID is <a href="${ origin }/reservation/${ trip.id }">${ trip.id }</a>. You can view or cancel your reservation on 330-cars.com with it.</p>
        <p>Thank you for renting with us! If you have any questions or concerns, call <a href="${ trip.company.phone }">${ trip.company.phone }</a> between 9AM-5PM 7 days a week.</p>
        <h2>Trip dates</h2>
        <table>
